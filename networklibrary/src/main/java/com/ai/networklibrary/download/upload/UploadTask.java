 ...
 */
package com.ai.networklibrary.download.upload;

import android.content.ContentValues;

import com.ai.networklibrary.db.UploadManager;
import com.ai.networklibrary.download.OkUpload;
...
import com.ai.networklibrary.request.base.ProgressRequestBody;
...
public class UploadTask<T> implements Runnable {
    ...
    public UploadTask<T> start() {
        ...
        if (progress.status != Progress.WAITING && progress.status != Progress.LOADING) {
            ...
            NetLogger.w("the task with tag " + progress.tag + " is already in the upload queue, current task status is " + progress.status);
        }
        return this;
    }
