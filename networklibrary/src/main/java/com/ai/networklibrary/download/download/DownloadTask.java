 ...
 */
package com.ai.networklibrary.download.download;

import android.content.ContentValues;
import android.text.TextUtils;

import com.ai.networklibrary.db.DownloadManger;
import com.ai.networklibrary.download.OkDownload;
import com.ai.networklibrary.download.task.PriorityRunnable;
import com.ai.networklibrary.exception.HttpException;
import com.ai.networklibrary.exception.OkGoException;
import com.ai.networklibrary.exception.StorageException;
...
public class DownloadTask implements Runnable {
    ...
    public DownloadTask save() {
        ...
        DownloadManger.getInstance().replace(progress);
        return this;
    }
    ...
    public void start() {
        if (OkDownload.getInstance().getTask(progress.tag) == null || DownloadManger.getInstance().get(progress.tag) == null) {
    ...
    public DownloadTask remove(boolean isDeleteFile) {
        ...
        DownloadManger.getInstance().delete(progress.tag);
    ...
    public void run() {
        ...
        if (code == 404 || code >= 500) {
            postOnError(progress, HttpException.NET_ERROR(code));
            return;
        }
    ...
    private void updateDatabase(Progress progress) {
        ...
        DownloadManger.getInstance().update(contentValues, progress.tag);
    }
}
